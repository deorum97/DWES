...
<div class="grid">
    <div class="card">
        ...
        <form id="articuloForm" autocomplete="off">
            <label>titulo (titulo)
                <input name="titulo" required maxlength="50" placeholder="titulo" value="<?php echo htmlspecialchars($articulo['titulo'] ?? ''); ?>">
            </label>

            <label>descripcion (descripcion)
                <input name="descripcion" required maxlength="50" placeholder="descripcion" value="<?php echo htmlspecialchars($articulo['descripcion'] ?? ''); ?>">
            </label>

            <label>foto (foto)
                <input name="foto" required maxlength="30" placeholder="foto" value="<?php echo htmlspecialchars($articulo['foto'] ?? ''); ?>">
            </label>

            <label>cosa (cosa)
                <input name="cosa" required maxlength="80" placeholder="cosa" value="<?php echo htmlspecialchars($articulo['cosa'] ?? ''); ?>">
            </label>

            <button type="submit">actualizar articulo (PUT JSON a la API)</button>
        </form>
